...
float power(float num, float n){int out=1; for(int i = 0; i < n; i++){out *= num;}return out;}
...
char* ftoa(float in, int dec){if(dec<0){return "";}if((int)(in*power(10,dec+1))%10>=5){in+=power(10, -dec);}int a = digit((int)in);char out[dec+1+a];char*temp = ita((int)in);for(int i = 0; i < a; i++){out[i]=temp[i];};out[a]='.';for(int i = 1; i <=dec; i++){in*=10;out[a+i]=itoc((int)in%10);};char* ret = out;return ret;}
